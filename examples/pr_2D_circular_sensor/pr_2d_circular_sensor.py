...
import cv2 # Import OpenCV for image decoding

import sys
import os

sys.path.remove("C:\\Users\\dsinden\\GitHub\\k-wave-python")
...
# # reset the initial pressure, ensures source has p0, but it has no effect
# source.p0 = np.zeros_like(kgrid_recon.x)

# # assign the time reversal data. Not reversed
# sensor.time_reversal_boundary_data = sensor_data.T

# # run the time-reversal reconstruction
# p0_recon = kspaceFirstOrder2D(medium=deepcopy(medium),
                              # kgrid=deepcopy(kgrid_recon),
                              # source=deepcopy(source),
                              # sensor=deepcopy(sensor),
                              # simulation_options=deepcopy(simulation_options),
                              # execution_options=deepcopy(execution_options))
...
sensor.time_reversal_boundary_data = interp_cart_data(kgrid=kgrid_recon,
...
p0_recon_interp = kspaceFirstOrder2D(medium=deepcopy(medium),
                                     kgrid=deepcopy(kgrid_recon),
                                     source=deepcopy(source),
                                     sensor=deepcopy(sensor),
                                     simulation_options=deepcopy(simulation_options),
...
fig, ax = plt.subplots()
im = plt.pcolormesh(np.squeeze(kgrid_recon.x_vec), 
                    np.squeeze(kgrid_recon.y_vec), p0_recon_interp['p_final'].T, 
                    cmap=cmap, vmin=-1, vmax=1)
ax.yaxis.set_inverted(True)
...
ax.plot(np.squeeze(kgrid_recon.y_vec) * 1e3, p0_recon_interp['p_final'][:, j], 'b-', label='Interpolated Reconstruction')
