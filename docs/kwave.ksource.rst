...
**Initial Pressure** (``p0``): Initial pressure distribution for initial value problems (e.g., photoacoustic imaging)::
...
**Velocity Sources** (``ux``, ``uy``, ``uz``, ``u_mask``): Particle velocity sources for specific applications.

**Stress Sources** (``sxx``, ``syy``, etc.): For elastic wave simulations.

Source Modes
------------

**Additive** (default): Source terms are added to the field equations. Most physically realistic.

**Dirichlet**: Source values are enforced as boundary conditions. Useful for specific modeling scenarios.
