...
def test_rotation_matrices():
    ...
    with warnings.catch_warnings():
        ...
        for _ in range(len(reader)):
            ...
            np.testing.assert_allclose(result, expected_matrix, rtol=1e-15, atol=1e-15,
                                     err_msg=f"Rx matrix mismatch for angle {theta}")
            
            # Test Ry
            result = Ry(theta)
            expected_matrix = reader.expected_value_of('Ry_matrix')
            np.testing.assert_allclose(result, expected_matrix, rtol=1e-15, atol=1e-15,
                                     err_msg=f"Ry matrix mismatch for angle {theta}")
            
            # Test Rz
            result = Rz(theta)
            expected_matrix = reader.expected_value_of('Rz_matrix')
            np.testing.assert_allclose(result, expected_matrix, rtol=1e-15, atol=1e-15,
                                     err_msg=f"Rz matrix mismatch for angle {theta}")
