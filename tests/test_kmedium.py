"""Tests for kWaveMedium: alpha_mode normalization and broader coverage."""
...
def test_is_defined_method():
    ...
    medium = kWaveMedium(sound_speed=1500, density=1000, alpha_coeff=0.75)
...
def test_absorbing_without_stokes_alpha_power_accepts_0d_ndarray():
    """The new type hint allows np.ndarray; a 0-d array must be accepted as scalar."""
    medium = kWaveMedium(sound_speed=1500, alpha_coeff=0.75, alpha_power=np.array(1.5))
...
def test_alpha_mode_validation_edge_cases():
    ...
    # Empty string is rejected at construction by _to_alpha_mode (added in #705)
    with pytest.raises(ValueError, match="must be an AlphaMode"):
        kWaveMedium(sound_speed=1500, alpha_mode="")
