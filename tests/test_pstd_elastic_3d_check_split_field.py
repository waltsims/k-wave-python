...
def test_pstd_elastic_3d_check_split_field():
    ...
    bowl_pos    = np.round(np.asarray(bowl_pos) / dx).astype(int) + np.asarray([Nx // 2, Ny // 2, Nz // 2])
    focus_pos   = np.round(np.asarray(focus_pos) / dx).astype(int) + np.asarray([Nx // 2, Ny // 2, Nz // 2])
