...
def test_pstd_elastic_2d_compare_binary_and_cartesian_sensor_mask():
    ...
    sound_speed_compression = 1500 * np.ones((Nx, Ny))  # [m/s]
    ...
    medium = kWaveMedium(sound_speed_compression,
                         ...
                         density=density)
    medium.sound_speed_shear[Nx // 2 - 1:, :] = 1200
    medium.sound_speed_compression[Nx // 2 - 1:, :] = 2000
    medium.density[Nx // 2 - 1:, :] = 1200
    ...
    sensor_data_c_ln = pstd_elastic_2d(deepcopy(kgrid),
                                       ...
                                       deepcopy(source),
                                       deepcopy(sensor),
                                       deepcopy(simulation_options_c_ln))
    ...
    simulation_options_c_nn = SimulationOptions(simulation_type=SimulationType.ELASTIC,
                                                cart_interp='nearest')
    sensor_data_c_nn = pstd_elastic_2d(deepcopy(kgrid),
                                       deepcopy(medium),
                                       deepcopy(source),
                                       deepcopy(sensor),
                                       deepcopy(simulation_options_c_nn))
    ...
    simulation_options_b = SimulationOptions(simulation_type=SimulationType.ELASTIC,
                                             cart_interp='linear')
    sensor_data_b = pstd_elastic_2d(deepcopy(kgrid),
                                    deepcopy(medium),
                                    deepcopy(source),
                                    deepcopy(sensor),
