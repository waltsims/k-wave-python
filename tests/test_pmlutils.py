...
def test_pml_sizes_2d():
    ...
    assert (options.pml_x_size == pml_size) and (options.pml_y_size == pml_size), "pml sizes incorrect when passing int"

    pml_sizes = [21, 22]
    simulation_options = SimulationOptions(pml_size=pml_sizes)
    options = SimulationOptions.option_factory(kgrid, simulation_options)
    assert (options.pml_x_size == pml_sizes[0]) and (options.pml_y_size == pml_sizes[1]), "pml sizes incorrect when passing list"

    pml_sizes = None
    pml_default: int = 20
    simulation_options = SimulationOptions()
    options = SimulationOptions.option_factory(kgrid, simulation_options)
    assert (options.pml_x_size == pml_default) and (options.pml_y_size == pml_default), "pml sizes incorrect when not defining sizes"
