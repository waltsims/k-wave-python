...
def extract_sensor_data(dim, sensor_data, file_index, sensor_mask_index, flags, record, p, ux_sgx, uy_sgy, uz_sgz=None):
    """
    ...
    if flags.binary_sensor_mask:
        ...
        if flags.record_p_max:
            if file_index == 1:
                ...
                sensor_data.p_max = np.max([sensor_data.p_max, p[sensor_mask_index]])
        ...
        if flags.record_p_min:
            if file_index == 1:
                ...
                sensor_data.p_min = np.min([sensor_data.p_min, p[sensor_mask_index]])
        ...
        if flags.record_u_max:
            if file_index == 1:
                ...
                match dim:
                    case 1:
                        sensor_data.ux_max = np.max([sensor_data.ux_max, ux_sgx[sensor_mask_index]])
                    case 2:
                        sensor_data.ux_max = np.max([sensor_data.ux_max, ux_sgx[sensor_mask_index]])
                        sensor_data.uy_max = np.max([sensor_data.uy_max, uy_sgy[sensor_mask_index]])
                    case 3:
                        sensor_data.ux_max = np.max([sensor_data.ux_max, ux_sgx[sensor_mask_index]])
                        sensor_data.uy_max = np.max([sensor_data.uy_max, uy_sgy[sensor_mask_index]])
                        sensor_data.uz_max = np.max([sensor_data.uz_max, uz_sgz[sensor_mask_index]])
        ...
        if flags.record_u_min:
            if file_index == 1:
                ...
                match dim:
                    case 1:
                        sensor_data.ux_min = np.min([sensor_data.ux_min, ux_sgx[sensor_mask_index]])
                    case 2:
                        sensor_data.ux_min = np.min([sensor_data.ux_min, ux_sgx[sensor_mask_index]])
                        sensor_data.uy_min = np.min([sensor_data.uy_min, uy_sgy[sensor_mask_index]])
                    case 3:
                        sensor_data.ux_min = np.min([sensor_data.ux_min, ux_sgx[sensor_mask_index]])
                        sensor_data.uy_min = np.min([sensor_data.uy_min, uy_sgy[sensor_mask_index]])
                        sensor_data.uz_min = np.min([sensor_data.uz_min, uz_sgz[sensor_mask_index]])
    ...
    # (record.tri and record.bc are the Delaunay triangulation and Barycentric coordinates returned by gridDataFast3D)
        ...
        if flags.record_p_max:
            if dim == 1:
                if file_index == 1:
                    ...
                    sensor_data.p_max = np.max([sensor_data.p_max, np.interp(record.grid_x, p, record.sensor_x)])

            else:
                if file_index == 1:
                    sensor_data.p_max = np.sum(p[record.tri] * record.bc, axis=1)
                else:
                    sensor_data.p_max = np.max([sensor_data.p_max, np.sum(p[record.tri] * record.bc, axis=1)])
        ...
        if flags.record_p_min:
            if dim == 1:
                if file_index == 1:
                    ...
                    sensor_data.p_min = np.min([sensor_data.p_min, np.interp(record.grid_x, p, record.sensor_x)])

            else:
                if file_index == 1:
                    sensor_data.p_min = np.sum(p[record.tri] * record.bc, axis=1)
                else:
                    sensor_data.p_min = np.min([sensor_data.p_min, np.sum(p[record.tri] * record.bc, axis=1)])
        ...
        if flags.record_u_max:
            if file_index == 1:
                ...
                match dim:
                    case 1:
                        sensor_data.ux_max = np.max([sensor_data.ux_max, np.interp(record.grid_x, ux_sgx, record.sensor_x)])
                    case 2:
                        sensor_data.ux_max = np.max([sensor_data.ux_max, np.sum(ux_sgx[record.tri] * record.bc, axis=1)])
                        sensor_data.uy_max = np.max([sensor_data.uy_max, np.sum(uy_sgy[record.tri] * record.bc, axis=1)])
                    case 3:
                        sensor_data.ux_max = np.max([sensor_data.ux_max, np.sum(ux_sgx[record.tri] * record.bc, axis=1)])
                        sensor_data.uy_max = np.max([sensor_data.uy_max, np.sum(uy_sgy[record.tri] * record.bc, axis=1)])
                        sensor_data.uz_max = np.max([sensor_data.uz_max, np.sum(uz_sgz[record.tri] * record.bc, axis=1)])
        ...
        if flags.record_u_min:
            if file_index == 1:
                ...
                match dim:
                    case 1:
                        sensor_data.ux_min = np.min([sensor_data.ux_min, np.interp(record.grid_x, ux_sgx, record.sensor_x)])
                    case 2:
                        sensor_data.ux_min = np.min([sensor_data.ux_min, np.sum(ux_sgx[record.tri] * record.bc, axis=1)])
                        sensor_data.uy_min = np.min([sensor_data.uy_min, np.sum(uy_sgy[record.tri] * record.bc, axis=1)])
                    case 3:
                        sensor_data.ux_min = np.min([sensor_data.ux_min, np.sum(ux_sgx[record.tri] * record.bc, axis=1)])
                        sensor_data.uy_min = np.min([sensor_data.uy_min, np.sum(uy_sgy[record.tri] * record.bc, axis=1)])
        ...
        match dim:
            case 1:
                if file_index == 1:
                    ...
                    sensor_data.p_max_all = np.max([sensor_data.p_max_all, p[record.x1_inside:record.x2_inside]])

            case 2:
                if file_index == 1:
                    sensor_data.p_max_all = p[record.x1_inside:record.x2_inside, record.y1_inside:record.y2_inside]
                else:
                    sensor_data.p_max_all = np.max([sensor_data.p_max_all, p[record.x1_inside:record.x2_inside, record.y1_inside:record.y2_inside]])
            ...
            case 3:
                if file_index == 1:
                    ...
                    sensor_data.p_max_all = np.max([sensor_data.p_max_all,
        ...
        match dim:
            case 1:
                if file_index == 1:
                    ...
                    sensor_data.p_min_all = np.min([sensor_data.p_min_all, p[record.x1_inside:record.x2_inside]])

            case 2:
                if file_index == 1:
                    sensor_data.p_min_all = p[record.x1_inside:record.x2_inside, record.y1_inside:record.y2_inside]
                else:
                    sensor_data.p_min_all = np.min([sensor_data.p_min_all, p[record.x1_inside:record.x2_inside, record.y1_inside:record.y2_inside]])
            ...
            case 3:
                if file_index == 1:
                    ...
                    sensor_data.p_min_all = np.min([sensor_data.p_min_all,
                                                    p[record.x1_inside:record.x2_inside, record.y1_inside:record.y2_inside, record.z1_inside:record.z2_inside]])
        ...
        match dim:
            case 1:
                if file_index == 1:
                    ...
                    sensor_data.ux_max_all = np.max([sensor_data.ux_max_all, ux_sgx[record.x1_inside:record.x2_inside]])
            ...
            case 3:
                if file_index == 1:
                    ...
                    sensor_data.uz_max_all = np.max([sensor_data.uz_max_all,
        ...
        match dim:
            case 1:
                if file_index == 1:
                    ...
                    sensor_data.ux_min_all = np.min([sensor_data.ux_min_all, ux_sgx[record.x1_inside:record.x2_inside]])
            ...
            case 2:
                if file_index == 1:
                    ...
                    sensor_data.ux_min_all = np.min([sensor_data.ux_min_all,
                                                     ux_sgx[record.x1_inside:record.x2_inside, record.y1_inside:record.y2_inside]])
                    sensor_data.uy_min_all = np.min([sensor_data.uy_min_all,
                                                     uy_sgy[record.x1_inside:record.x2_inside, record.y1_inside:record.y2_inside]])
            ...
            case 3:
                if file_index == 1:
                    ...
                    sensor_data.uy_min_all = np.min([sensor_data.uy_min_all,
                                                     uy_sgy[record.x1_inside:record.x2_inside, record.y1_inside:record.y2_inside, record.z1_inside:record.z2_inside]])
                    sensor_data.uz_min_all = np.min([sensor_data.uz_min_all,
                                                     uz_sgz[record.x1_inside:record.x2_inside, record.y1_inside:record.y2_inside, record.z1_inside:record.z2_inside]])
