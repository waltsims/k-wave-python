...
def angular_spectrum(
    ...
    print('Running angular spectrum projection...')
    print(f"  start time: {TicToc.start_time}")
    print(f"  input plane size: {Nx} by {Ny} grid points ({scale * Nx * dx} by {scale * Ny * dx} {prefix}m)")
    print(f"  grid expansion: {grid_expansion} grid points")
    ...
    print(f"  maximum supported frequency: {scale_SI(c0 / (2 * dx))}Hz")
    print(f"  input signal length: {Nt} time points ({scale_SI(Nt * dt)}s)")
    ...
    # Update command line status
    print(f'  precomputation completed in {scale_time(TicToc.toc())}')
    print('  starting z-step loop...')
    ...
    for z_index in range(Nz):
        ...
        if z == 0:
            ...
            if z_index == loops_for_time_est:
                ...
                print(f'  estimated simulation time {est_sim_time}  ...')

    # update command line status
    print(f'  simulation completed in {time.perf_counter() - TicToc.start_time}')
