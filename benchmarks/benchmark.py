...
def run(
    ...
    *,
    ...
    result: dict[str, Any] = {
        "comp_size": [],
        "comp_time": [],
        "options": _options_payload(options, backend, device),
    ...
    for nx, ny, nz, scale in cases:
        ...
        try:
            ...
            for loop_num in range(1, options.num_averages + 1):
                ...
                loop_time = _rolling_average(loop_time, elapsed_time, loop_num)
                if options.report_mem_usage:
                    loop_mem_usage = _rolling_average(loop_mem_usage, _peak_memory_bytes(), loop_num)
                _store_case_result(result, nx * ny * nz, loop_time, loop_mem_usage, options.report_mem_usage)
                _save_results(path, result)
...
def _save_results(path: Path, result: dict[str, Any]) -> None:
...
def _options_payload(options: BenchmarkOptions, backend: str, device: str) -> dict[str, Any]:
